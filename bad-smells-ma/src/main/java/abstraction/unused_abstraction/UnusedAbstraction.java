...
public class UnusedAbstraction extends AbstractAnalyzer {
    ...
    private Set<String> extract_unused_language_types(Driver driver) {
        try (Session session = driver.session()) {
            ...
            return session.readTransaction(tx -> {
                var res = tx.run("MATCH (l:LanguageClass)"+
                        "WHERE NOT (l)-[:uses]-(:AnalyzerClass)"+
                        "RETURN l");
                while(res.hasNext()){
                    unused_language_types.add(res.next().get("l").asNode().get("name").asString());
                }
    ...
    private void init_database(ModularLanguage language, SimulatorModel simulatorModel, Driver driver)  {
        try (Session session = driver.session()) {
            ...
            languageNodes.stream().filter(this::is_regular_type).forEach(languageNode -> create_language_nodes(session, languageNode));
            analyzerNodes.forEach(analyzerNode -> create_language_nodes_and_relations_to_analyzer_nodes(session, analyzerNode));
        }
    }

    private boolean is_regular_type(CtType<?> type){
        if(type.getQualifiedName().contains(".impl.") && type.getQualifiedName().endsWith("Impl")) return false;
        if(type.getQualifiedName().contains(".util.") && type.getQualifiedName().endsWith("Factory")) return false;
        if(type.getQualifiedName().contains(".util.") && type.getQualifiedName().endsWith("Switch")) return false;
        return true;
    }
    ...
    private Report createReport(Set<String> unused_language_types) {
        ...
        description.append(String.format("%s unused types found +\n\n", unused_language_types.size()));
        unused_language_types.forEach(type -> {
            description.append(
                    String.format("The language type %s is unused.\n", type)
            );
        });
        return new Report(title, description.toString(), !unused_language_types.isEmpty(), unused_language_types.size());
    }
