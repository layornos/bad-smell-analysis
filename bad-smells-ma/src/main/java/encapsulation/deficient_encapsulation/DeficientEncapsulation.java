package encapsulation.deficient_encapsulation;
...
public class DeficientEncapsulation extends AbstractAnalyzer {
    ...
    protected Report fullAnalysis(
        ...
        Set<CtType<?>> analysisTypes = getAllTypes(simulatorModel);
        for (CtType<?> analysisType : analysisTypes) {
            for (CtMethod<?> method : analysisType.getMethods()) {
        ...
        return new Report(getName(), sb.toString(), sb.length() > 0, map.keySet().size());
    }
