...
public class FoldedHierarchyNeo4j {
    ...
    public void start(Project project) {
        ...
        // NORMAL layer violation: MATCH (a:AnalyzerClass)-[]->(b:LanguageClass) where
        // a.layer_number < b.layer_number return a,b
        // STRICT layer violation: MATCH (a:AnalyzerClass)-[]->(b:LanguageClass) where
        // (a.layer_number - b.layer_number) > 0 return a,b
            ...
            var nodesNormalLayerViolation =
                    session.readTransaction(
                            tx -> {
                                var result =
                                        tx.run(
                                                "MATCH (a:AnalyzerClass)-[]->(b:LanguageClass) where a.layer_number < b.layer_number return a.name,b.name");
            ...
            var nodesStrictLayerViolation =
                    session.readTransaction(
                            tx -> {
                                var result =
                                        tx.run(
                                                "MATCH (a:AnalyzerClass)-[]->(b:LanguageClass) where (a.layer_number - b.layer_number) > 0 return a.name,b.name");
    ...
    private void create_analyzer_nodes(Session session, CtType<?> type) {
        ...
        session.writeTransaction(
                tx -> {
                    tx.run(
                            "CREATE (a:AnalyzerClass {name:$fqn,layer:$layer,layer_number:$layer_number})",
                            parameters(
                                    "fqn",
    ...
    private void create_language_nodes(Session session, CtTypeReference<?> v) {
        ...
        session.writeTransaction(
                tx -> {
                    tx.run(
                            "Merge (a:LanguageClass {name:$fqn,layer:$layer,layer_number:$layer_number})",
                            parameters(
                                    "fqn",
                                    v.getQualifiedName(),
                                    "layer",
